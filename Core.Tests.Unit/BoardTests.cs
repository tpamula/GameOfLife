...
namespace Core.Tests.Unit
{
    public class BoardTests
    {
        ...
        {
            var board = new Board();
            board[0, 1] = board[0, 2] = board[0, 3] = board[1, 1] = board[1, 2] = CellType.Alive;

            board.NextGeneration();

            board[1, 2].ShouldBe(CellType.Dead);
        }

        [Fact]
        public void should_die_with_seven_neighbors()
        {
            var board = new Board();
            board[0, 1] = board[0, 2] = board[0, 3] = board[1, 1] = board[1, 2] = board[1, 3] = board[2, 1] = board[2, 2] = CellType.Alive;
